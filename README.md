...
> 協議の結果、LOLA由来データの利用は正式に許可（ただし生DEM処理は非推奨、前処理済みカタログを優先）、
> 情報Ⅰの範囲は標準ノートブックで引き続き厳守し高度な統計・回帰は発展ノートブックに分離、
> という方針で合意している。DS-4〜8はいずれも「要検証」または「設計変更済み」の段階で、
> 実データ確認前の着手はしない（Ver.1.0のDS-1〜3で確認された「報告書記載値と実測値の乖離」を
> 繰り返さないため）。
