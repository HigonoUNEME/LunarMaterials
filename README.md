...
> **スコープの原則**：このリポジトリで扱うデータセットは、以下の3つに固定する。
> 追加のデータセット（LOLA・LAMP・LEND・生の衛星画像 等）は、今回の教材開発の
> 範囲には含めない。拡張の可能性については、報告書の「今後の展望」にのみ記述する。
...
> 2026-08-27時点：実データ確認の結果、Robbins DBに深さ（Depth）の列が存在しないことが判明したため、
> 当初案の「直径と深さの関係」は削除し、1番を「直径と離心率・扁平率の関係」に差し替えた。

1. クレーターの直径と、離心率・扁平率（＝どれくらい丸いか）の関係
2. クレーターの空間密度（緯度経度分布のかたより）
3. 緯度と正午の温度の関係
4. 同一地点の昼夜の温度差（正午と深夜0時の温度差）
5. クレーターの推定年代と、直径・分布との関係（DeepCraters追加により可能）
...
- LOLA・LAMP・LEND等、他のLRO搭載機器のデータ
- 生の衛星画像（GeoTIFF等）の直接処理
- 外部APIへのライブ接続、バックエンドサーバーの構築・運用
- 機械学習・AIによる予測（今後の展望として報告書にのみ記述し、本教材には実装しない）

## 8. 今後の展望（本教材には含まないが、報告書に記述する内容）

- 他の観測データ（LOLA地形データ等）を組み合わせたさらなる拡張
